...
```bash
git clone https://github.com/[your-username]/mymedbuddy.git
cd mymedbuddy
```
...
Sets up the app with `ProviderScope` and `MultiProvider` for state management. Uses `FutureBuilder` to check for user data and route to `OnboardingScreen` or `HomeScreen`.
...
### State Management (`medication_provider.dart`)

Manages medication CRUD operations and dose tracking, persisting data to `shared_preferences`.

```dart
Future addMedication(MedicationModel medication) async {
...
### UI Rendering (`medicationschedule_screen.dart`)
...
### Data Persistence (`shared_prefs_service.dart`)

Saves/retrieves data as JSON strings for persistence.

```dart
static Future saveUserData({String? name, int? age, List? medications}) async {
...
```dart
static Future getHealthTip() async {
  try {
